...
public class Instrument extends InstructionListUtils implements ClassFileTransformer {
  ...
  // Public so it can be enabled from daikon.dcomp.Instrument.
        ...
        synchronized (this) {
          ...
          for (InstructionHandle ih = il.getStart(); ih != null; ) {
            ...
            InstructionList new_il =
                generate_return_instrumentation(inst, mgen, exitLocationIter, shouldIncludeIter);
   ...
   *
   * @param instructions instruction list a method
  ...
  private @Nullable MethodInfo create_method_info(ClassInfo classInfo, MethodGen mgen) {
    ...
    if (debugInstrument.enabled) {
      ...
      for (String paramName : paramNames) {
        debugInstrument.log("param: %s%n", paramName);
      }
    }
